...
import parking_lot.entities.*;
import parking_lot.enums.VehicleSize;
import parking_lot.strategy.VehicleBasedFeeStrategy;
...
public class ParkingLotDemo {
    public static void main(String[] args) {
        ...
        Optional<ParkingTicket> bikeTicketOpt = Optional.ofNullable(parkingLot.parkVehicle(bike));

        Optional<ParkingTicket> carTicketOpt = Optional.ofNullable(parkingLot.parkVehicle(car));

        Optional<ParkingTicket> truckTicketOpt = Optional.ofNullable(parkingLot.parkVehicle(truck));
        ...
        Optional<ParkingTicket> car2TicketOpt = Optional.ofNullable(parkingLot.parkVehicle(car2));

        // 4. Simulate a vehicle entry that fails (no available spots)
        Vehicle bike2 = new Bike("B-000");
        try {
            Optional<ParkingTicket> failedBikeTicketOpt = Optional.ofNullable(parkingLot.parkVehicle(bike2));
        }
        catch (Exception e ) {}
        ...
        if (carTicketOpt.isPresent()) {
            Optional<Double> feeOpt = Optional.of(parkingLot.unParkVehicle(car.getLicenseNumber()));
