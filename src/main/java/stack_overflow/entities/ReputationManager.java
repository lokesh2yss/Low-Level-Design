...
public class ReputationManager implements PostObserver {
    ...
    public void onPostEvent(Event event) {
        ...
        switch (event.getType()) {
            case UPVOTE_QUESTION -> postAuthor.updateReputation(QUESTION_UPVOTE_REP);
            ...
            case UPVOTE_ANSWER -> postAuthor.updateReputation(ANSWER_UPVOTE_REP);
            case DOWNVOTE_ANSWER -> {
                postAuthor.updateReputation(DOWNVOTE_REP_PENALTY);
                event.getActor().updateReputation(POST_DOWNVOTED_REP_PENALTY);
            }
            case ACCEPT_ANSWER -> postAuthor.updateReputation(ACCEPTED_ANSWER_REP);
        }
    }
}
